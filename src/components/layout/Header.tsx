      ...
      <div className="header-content">
        ...
        <div ref={menuRef}>
          <nav className={`nav ${isMenuOpen ? "open" : ""}`}>
            <Link to="/" className={`bn5 ${isActive('/') ? 'active' : ''}`} onClick={() => handleNavigation('/')}>
              {t('header.home')}
            </Link>
            <Link to="/about" className={`bn5 ${isActive('/about') ? 'active' : ''}`} onClick={() => handleNavigation('/about')}>
              {t('header.about')}
            </Link>
            <Link to="/projects" className={`bn5 ${isActive('/projects') ? 'active' : ''}`} onClick={() => handleNavigation('/projects')}>
              {t('header.projects')}
            </Link>
            <Link to="/skills" className={`bn5 ${isActive('/skills') ? 'active' : ''}`} onClick={() => handleNavigation('/skills')}>
              {t('header.skills')}
            </Link>
            <Link to="/contact" className={`bn5 ${isActive('/contact') ? 'active' : ''}`} onClick={() => handleNavigation('/contact')}>
              {t('header.contact')}
            </Link>
          </nav>
        </div>
